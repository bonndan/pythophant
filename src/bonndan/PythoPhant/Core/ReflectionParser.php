 ...
 */
class ReflectionParser implements Parser
{
    /**
     * @var TokenList
     */
        ...
        foreach ($this->lines as $line) {
            ...
            if ($first->getNestingLevel() == 1) {
                ...
                if ($second instanceof DocCommentToken) {
                    ...
                    $declaration = $second;
                    if ($docComment === null) {
                        throw new \PythoPhant\Exception(
    ...
    private function handleElementDeclaration(array $line, DocCommentToken $docComment)
    {
        ...
        if (!$docComment->isMethodComment()) {
            if ($line[1]->getContent() == 'const') {
                $type = "PythoPhant\Reflection\ClassConst";
                $setter = 'addConstant';
                $name = $line[3]->getContent(); /* @todo not nice */
            } else {
                $type = "PythoPhant\Reflection\ClassVar";
                $setter = 'addVar';
            }
            
        } else {
            $type = "PythoPhant\Reflection\Method";
            $setter = 'addMethod';
        }
        ...
        foreach ($line as $i => $token) {
             ...
             */
            if ($token->getContent() == Grammar::T_ASSIGN) {
